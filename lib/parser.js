...
class Parser extends BaseParser {
   ...
   */
  module() {
    // annotation -> import -> extend -> module body
    // 不知道为什么要把所有 comment 给整理出来，感觉是为了后面生成注释？
    var annotation;
  ...
  moduleBody() {
    ...
    while (this.look.tag) {
      ...
      /**
       * @TODO 为什么有些使用 is，有些使用 isWord 来判断，感觉是 Tag 一开始没有定义完全
       */
  ...
  exprItem() {
    /**
     * @todo 这里表示一个 expression item，为什么有一个 !
     */
    if (this.look.tag === '!') {
      return this.notExpr();
    }
  ...
  fun() {
    ...
    // static async function(参数列表) throws : 返回值类型 {
    //   预发语句
    // }
    const begin = this.getIndex();
    let isStatic = false;
